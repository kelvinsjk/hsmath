...
export default class Expression {
  ...
  toString(options?: toStringOptions): string {
    ...
    let firstTerm = true, outputString = '';
    for (const term of this.terms) {
      const variable = term.toString(termOptions);
      if (variable !== '0') { // only add to output for non-zero terms
        if (firstTerm) {
          outputString += `${variable}`;
          firstTerm = false;
        } else { // non-first term: add + sign if no - signs found
          if (variable[0] !== '-') {
            outputString += ` + ${variable}`
          } else {
            outputString += ` ${variable}`;  

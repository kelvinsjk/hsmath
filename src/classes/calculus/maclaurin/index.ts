...
function sin(r: number, options?: MaclaurinOptions): PolynomialFn {
  ...
  for (let i = 0; i <= r; i++) {
    if (i % 2 === 1) {
      const j = (i - 1) / 2;
      const coeff = new Fraction(-1).pow(j).divide(factorial(2*j+1)).times(a.pow(2*j+1));
      coeffArray.push(coeff);
    } else {
      coeffArray.push(0);
    }
...
function cos(r: number, options?: MaclaurinOptions): PolynomialFn {
  ...
  for (let i = 0; i <= r; i++) {
    if (i % 2 === 0) {
      const j = i / 2;
      const coeff = new Fraction(-1).pow(j).divide(factorial(2*j)).times(a.pow(2*j));
      coeffArray.push(coeff);
    } else {
      coeffArray.push(0);
    }
...
function ln(r: number, options?: MaclaurinOptions): PolynomialFn {
  ...
  for (let i = 1; i <= r; i++) {
    const coeff = new Fraction(-1).pow(i+1).divide(i).times(a.pow(i));
    coeffArray.push(coeff);
  }
  return new PolynomialFn(coeffArray, { variableAtom, ascending: true })
}

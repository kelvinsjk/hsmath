...
test('doubleAngle', () => {
  ...
  expect(`${integrate.cosSquare({ limits: [piOver3, piOver2] })}`).toBe('- \\frac{1}{8} \\sqrt{3} + \\frac{1}{12} \\pi');
...
test('factor formula', () => {
  ...
  expect(`${integrate.sinCos({ a: new Fraction(2), limits: [0, 45] }, { a: 1 })}`).toBe('- \\frac{1}{6} \\sqrt{2} + \\frac{2}{3}');
  expect(`${integrate.sinCos({ a: new Fraction(2), limits: [piOver4, piOver3] }, { a: 1 })}`).toBe('- \\frac{1}{12} + \\frac{1}{6} \\sqrt{2}');

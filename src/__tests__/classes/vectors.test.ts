...
test('vectors 1', () => {
  ...
  expect(`${v3.plus(v4).toString({ ijkMode: true })}`).toBe(
    '\\frac{1}{2} ( 2 \\mathbf{i} - 4 \\mathbf{j} + 6 \\mathbf{k} )',
  );
  expect(`${v3.plus(v4).toString({ ijkMode: true, multiplyKIn: true })}`).toBe(
...
test('line', () => {
  expect(`${Line.X}`).toBe(
    '\\mathbf{r} = \\begin{pmatrix} 0 \\\\ 0 \\\\ 0 \\end{pmatrix} + \\lambda \\begin{pmatrix} 1 \\\\ 0 \\\\ 0 \\end{pmatrix}',
  );
  expect(`${Line.Y}`).toBe(
    '\\mathbf{r} = \\begin{pmatrix} 0 \\\\ 0 \\\\ 0 \\end{pmatrix} + \\lambda \\begin{pmatrix} 0 \\\\ 1 \\\\ 0 \\end{pmatrix}',
  );
  expect(`${Line.Z}`).toBe(
    '\\mathbf{r} = \\begin{pmatrix} 0 \\\\ 0 \\\\ 0 \\end{pmatrix} + \\lambda \\begin{pmatrix} 0 \\\\ 0 \\\\ 1 \\end{pmatrix}',
  );
  ...
  expect(`${l1Clone}`).toBe(
    '\\mathbf{r} = \\begin{pmatrix} 2 \\\\ 0 \\\\ - 1 \\end{pmatrix} + \\lambda \\begin{pmatrix} 1 \\\\ 3 \\\\ 0 \\end{pmatrix}',
  );
  expect(`${l1}`).toBe(
    '\\mathbf{r} = \\begin{pmatrix} 2 \\\\ 0 \\\\ - 1 \\end{pmatrix} + \\lambda \\begin{pmatrix} 1 \\\\ 3 \\\\ 0 \\end{pmatrix}',
  );
  expect(`${l1.toString({ lambda: 's' })}`).toBe(
    '\\mathbf{r} = \\begin{pmatrix} 2 \\\\ 0 \\\\ - 1 \\end{pmatrix} + s \\begin{pmatrix} 1 \\\\ 3 \\\\ 0 \\end{pmatrix}',
  );
  expect(`${l1.toString({ ijkMode: true })}`).toBe(
    '\\mathbf{r} = \\left( 2 \\mathbf{i} - \\mathbf{k} \\right) + \\lambda \\left( \\mathbf{i} + 3 \\mathbf{j} \\right)',
  );
  expect(`${l1.toString({ mode: 'combined' })}`).toBe(
    '\\begin{pmatrix} 2 + \\lambda \\\\ 3 \\lambda \\\\ - 1 \\end{pmatrix}',
  );
...
test('plane', () => {
  ...
  expect(`${p3.toString({ ijkMode: true })}`).toBe(
    '\\mathbf{r} \\cdot \\left( 2 \\mathbf{i} + \\mathbf{j} - \\mathbf{k} \\right) = 5',
  );
  ...
  expect(`${new Plane(new Vector(0, new Fraction(1, 2), 0), { d: new Fraction(1, 3) }).point().toCoordinates()}`).toBe(
    '\\left( 0 , \\frac{2}{3} , 0 \\right)',
  );
  expect(`${new Plane(new Vector(0, 0, -3), { d: 1 }).point().toCoordinates()}`).toBe(

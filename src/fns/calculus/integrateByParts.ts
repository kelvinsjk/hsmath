    ...
    if (u.n.isEqual(0)) {
      ...
      if (limits === undefined) {
        ...
        if (v instanceof ExpFn) {
          ...
          firstTerm = new Expression(v.valueAt(limits[1] as Fraction | number).multiply(u.coeff)).multiply(v.coeff);
          secondTerm = new Expression(v.valueAt(limits[0] as Fraction | number).multiply(u.coeff)).multiply(v.coeff);
        } else {
      ...
      if (limits === undefined) {
        ...
        const vTerm1 =
          v instanceof ExpFn ? v.valueAt(limits[1] as number | Fraction) : v.valueAt(limits[1] as number | Angle);
        const vTerm2 =
          v instanceof ExpFn ? v.valueAt(limits[0] as number | Fraction) : v.valueAt(limits[0] as number | Angle);

 ...
 * 
 */
function productRule(f: CalculusFn, g: CalculusFn, x: number | Fraction): Fraction { //| Exp | SquareRoot 
  return f.derivative().valueAt(x).times(g.valueAt(x)).plus(f.valueAt(x).times(g.derivative().valueAt(x)));
}
 ...
 * 
 */
function quotientRule(f: CalculusFn, g: CalculusFn, x: number | Fraction): Fraction { //| Exp | SquareRoot 
  const num = f.derivative().valueAt(x).times(g.valueAt(x)).minus(f.valueAt(x).times(g.derivative().valueAt(x)));
